...
- Build-time **CUDA toolkit** (`nvcc`) is installed so `hy3dgen/texgen/custom_rasterizer` compiles during the image build (the PyTorch runtime base image does not include `nvcc`).
